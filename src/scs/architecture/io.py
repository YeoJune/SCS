...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, key_padding_mask: Optional[Tensor] = None,
        ...
        # =========================================================================
        # START: `batch_first` 처리 로직 (가장 중요)
        # =========================================================================
        # `batch_first`가 True이면, 모든 연산을 위해 (S, B, E) 형태로 바꾼다.
        if self.batch_first and is_batched:
            # key, value가 query와 같은 객체인지 확인하여 불필요한 연산을 피함
            if key is value:
                if query is key:
        ...
        if self._qkv_same_embed_dim:
            ...
            if self.in_proj_bias is None: b_q = b_k = b_v = None
            else: b_q, b_k, b_v = self.in_proj_bias.chunk(3)
            q = F.linear(query, self.q_proj_weight, b_q)
            k = F.linear(key, self.k_proj_weight, b_k)
            v = F.linear(value, self.v_proj_weight, b_v)

        # 4. Reshape q, k, v for multihead attention -> (B*H, L, E_head) or (B*H, S, E_head)
        ...
        # is_causal 처리 (attn_mask가 없을 때만 동작)
        if is_causal and attn_mask is None:
            if query.size(1) != key.size(1):
                 raise ValueError("is_causal only supports self-attention.")
        ...
        if attn_mask is not None:
            if attn_mask.dtype == torch.bool:
                # attn_mask가 (L,S) 또는 (H*B, L,S) 이므로 bmm 결과 (H*B, L,S)에 맞게 unsqueeze
                if attn_mask.dim() == 2:
        ...
        # (B*H, L, E_head) -> (L, B*H, E_head) -> (L, B, D)
        ...
        # =========================================================================
        # START: `batch_first` 처리 복원 로직
        # =========================================================================
        # 연산이 끝났으므로, 원래 `batch_first` 형태에 맞춰 결과를 transpose
        if self.batch_first and is_batched:
            attn_output = attn_output.transpose(1, 0)
        # =========================================================================
        # END: `batch_first` 처리 복원 로직
        # =========================================================================
            
        if not is_batched:
            attn_output = attn_output.squeeze(1) # unbatched 입력의 batch 차원 제거
