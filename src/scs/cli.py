    ...
    try:
        ...
        with torch.no_grad():
            ...
            for clk in range(max_clk):
                ...
                # 외부 입력 적용
                if clk < input_tokens.shape[1]:
                    current_token = input_tokens[:, clk]
                    if attention_mask is None or attention_mask[:, clk].item():
                        external_input = model.input_interface(current_token.unsqueeze(-1))
                        if external_input is not None:
                            model.nodes[model.input_node].membrane_potential += external_input[0]
                
                # 상태 업데이트
                model._phase2_update_states(None, current_spikes)
