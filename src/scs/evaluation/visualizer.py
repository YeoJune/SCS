...
class SCSVisualizer:
    ...
    def _create_gate_view(
        ...
        im = ax.imshow(gate_grid, cmap='viridis', aspect='equal', vmin=0.0, vmax=1.0)
    ...
    def _create_bias_view(
        ...
        im = ax.imshow(bias_grid, cmap='viridis', aspect='equal', vmin=0.0, vmax=1.0)
    ...
    def _create_source_fixed_view(
        ...
        im = ax.imshow(full_view, cmap='viridis', aspect='equal', vmin=0.0, vmax=1.0)
    ...
    def collect_spike_patterns(
        ...
        for t_block in range(max_t_blocks):
            try:
                # === T-block 시작: 입력 확률 계산 ===
                input_probs = model._get_external_input_at_clk(
                ...
                if model.timing_manager.all_ended:
                    logger.info(f"T-block {t_block}에서 처리 완료")
                    break
    ...
    def generate_all_visualizations(self, model, test_loader, output_dir: Path):
        ...
        try:
            ...
            model.eval()
            with torch.no_grad():
                # 1. 스파이크 패턴 수집 (T-block 기반)
                all_spike_patterns = self.collect_spike_patterns(model, input_tokens, attention_mask, max_t_blocks=128)
